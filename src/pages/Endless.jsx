...
export default function Endless() {
  ...
  const {
    ...
    current, hintsData, submitGuess, restart, comboMultiplier,
        ...
        <div className="card" style={{ maxWidth: 420, textAlign: 'center' }}>
          ...
          <div className="grid-2" style={{ marginBottom: 'var(--space-6)' }}>
            <div className="stat-card">
              <div className="stat-value" style={{ fontSize: '2.2rem' }}>{score}</div>
              <div className="stat-label">Score final</div>
            </div>
            <div className="stat-card">
              <div className="stat-value" style={{ fontSize: '2.2rem' }}>{rounds}</div>
              <div className="stat-label">Rounds</div>
            </div>
            <div className="stat-card">
              <div className="stat-value" style={{ fontSize: '2.2rem' }}>{maxCombo}</div>
              <div className="stat-label">Meilleur combo</div>
            </div>
            <div className="stat-card">
              <div className="stat-value" style={{ fontSize: '1.6rem' }}>x{comboMultiplier.toFixed(2)}</div>
              <div className="stat-label">Multiplicateur max</div>
            </div>
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
            <button className="btn btn-primary btn-lg" onClick={restart}>🔄 Rejouer</button>
            <Link to="/leaderboard" className="btn btn-ghost">🏆 Voir le classement</Link>
      ...
      <div className="game-header">
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 'var(--space-4)', flexWrap: 'wrap', marginTop: 'var(--space-3)' }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Score</div>
          </div>

          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '1.5rem', fontWeight: 800, color: combo > 0 ? 'var(--warning)' : 'var(--text-muted)', fontFamily: 'var(--font-display)', animation: combo > 1 ? 'combo-burst 400ms' : 'none' }}>
              x{comboMultiplier.toFixed(2)}
            </div>
            <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Combo {combo > 0 && `(${combo})`}</div>
          </div>
          ...
          <div style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Vies</div>
          </div>

          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: '1.5rem', fontWeight: 800, color: 'var(--text-secondary)', fontFamily: 'var(--font-display)' }}>{rounds}</div>
            <div style={{ fontSize: '0.7rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Rounds</div>
          </div>
        </div>
      </div>

      {/* Feedback round terminé */}
      {current.finished && (
        <div style={{
          textAlign: 'center',
          padding: 'var(--space-3)',
          ...
          color: current.won ? 'var(--success)' : 'var(--error)',
          fontWeight: 700,
        }}>
          {current.won
            ? `✅ ${current.name} — Prochain round...`
            : `❌ C'était ${current.name} — Prochain round...`}
        </div>
      )}
        ...
        <div className="hints-grid">
          {Array.from({ length: 6 }, (_, i) => i + 1).map(n => (
            <HintCard
              ...
              isRevealed={current.hintsRevealed?.includes(n)}
              isNew={current.hintsRevealed?.[current.hintsRevealed.length - 1] === n && n > 1}
            />
          ))}
        </div>
